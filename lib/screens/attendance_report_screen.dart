...
// ======================
// Attendance Report Screen — Version 3 — 2025-08-26
// ======================
// تغييرات هذه النسخة:
// - إضافة البحث بالاسم/الـUID في الشاشة + RAW + التصدير
// - تصدير عمود OT بالدقائق (عدد صحيح) بدل HH:MM
// - الحفاظ على نفس منطق التجميع (overnight تم دعمه في 2.1)
// ======================
...
import '../utils/export.dart';       // Export.buildExcelFromSummaries (تفترض كتابة القيم كما تُمرَّر)
...
class _AttendanceReportScreenState extends State<AttendanceReportScreen> {
  // لابل الإصدار (يظهر داخل الـAppBar)
  static const String _versionLabel = 'Version 3 — 2025-08-26';
  ...
  // ======== حساب صفوف التصدير (مع دعم البحث) ========
  List<_ExportRow> _computeRowsForExport(List<QueryDocumentSnapshot<Map<String, dynamic>>> docs) {
    ...
    final filtered = docs.where((d) {
      ...
      if (qtxt.isNotEmpty) {
        ...
        if (!(uid.contains(qtxt) || name.contains(qtxt))) return false;
      }
  ...
  // ======== تصدير XLSX باستخدام Export.buildExcelFromSummaries ========
  Future<void> _exportExcel() async {
    try {
      ...
      // خريطة الصفوف للإكسيل (لاحظ: عمود OT بالدقائق كرقم)
      final mapped = rows.map((r) {
        final displayBranch = r.branchName.isNotEmpty ? r.branchName : (_branchNames[r.branchId] ?? '—');
        final displayShift  = r.shiftName.isNotEmpty  ? r.shiftName  : (_shiftNames[r.shiftId] ?? '—');
  ...
  Widget _rowTile(_DaySummary r) {
    ...
    final displayBranch = r.branchName.isNotEmpty
        ? r.branchName
        : (_branchNames[r.branchId] ?? '—');
    final displayShift = r.shiftName.isNotEmpty
        ? r.shiftName
        : (_shiftNames[r.shiftId] ?? '—');
